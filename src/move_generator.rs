...
pub fn generate_all_moves(game: &GameState) -> ListOfMoves {
    ...
    for i in 0..7 {
        for j in 0..7 {
            let location = (i, j);
            if let Some(moves_till_collide) = match game.piece_register.view(location) {
                Some(piece_record) => {
                    if piece_record.affiliation == game.turn {
                        match piece_record.class {
                            crate::types::Class::Pawn => {
                                Some(generate_pawn_moves_till_collide(game, &location))
                            }
                            crate::types::Class::Knight => {
                                Some(generate_knight_moves_till_collide(game, &location))
                            }
                            crate::types::Class::Bishop => {
                                Some(generate_bishop_moves_till_collide(game, &location))
                            }
                            crate::types::Class::Rook => {
                                Some(generate_rook_moves_till_collide(game, &location))
                            }
                            crate::types::Class::Queen => {
                                Some(generate_queen_moves_till_collide(game, &location))
                            }
                            crate::types::Class::King => {
                                Some(generate_king_moves_till_collide(game, &location))
                            }
                        }
                    } else {
                        None
