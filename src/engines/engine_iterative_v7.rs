//! Iterative-deepening material-search engine (V7).
//!
...
//!
//! V7 marker:
//! - Successor to V6 with richer move-ordering memory.
//! - Adds countermove and continuation-history ordering in the search backend.
...
use crate::search::iterative_deepening_v7::{
...
impl Engine for IterativeEngine {
    ...
    fn choose_move(
        ...
        if let Some(best) = chosen {
            ...
            if preferred != best {
                out.info_lines
                    .push("info string iterative_engine_v7 queen_promotion_preferred".to_owned());
            }
        ...
        out.info_lines.push(format!(
            "info string iterative_engine_v7 default_depth {}",
            self.default_depth
        ));
        out.info_lines.push(format!(
            "info string iterative_engine_v7 scorer {:?}",
            self.scorer_kind
        ));
        out.info_lines.push(format!(
            "info string iterative_engine_v7 used_depth {}",
            depth
        ));
        if let Some(ms) = params.movetime_ms {
            out.info_lines.push(format!(
                "info string iterative_engine_v7 movetime_ms {}",
                ms
            ));
        }
        out.info_lines.push(format!(
