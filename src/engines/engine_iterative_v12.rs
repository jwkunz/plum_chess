//! Iterative-deepening material-search engine (V12).
//!
...
//!
//! V12 marker:
//! - Successor to V11 with stronger tactical leaf search.
//! - Uses deeper quiescence + SEE-informed tactical filtering.
...
impl Engine for IterativeEngine {
    ...
    fn choose_move(
        ...
        if let Some(best) = chosen {
            ...
            if preferred != best {
                out.info_lines
                    .push("info string iterative_engine_v12 queen_promotion_preferred".to_owned());
            }
        ...
        out.info_lines.push(format!(
            "info string iterative_engine_v12 default_depth {}",
            self.default_depth
        ));
        out.info_lines.push(format!(
            "info string iterative_engine_v12 scorer {:?}",
            self.scorer_kind
        ));
        out.info_lines.push(format!(
            "info string iterative_engine_v12 used_depth {}",
            depth
        ));
        if let Some(ms) = effective_params.movetime_ms {
            out.info_lines.push(format!(
                "info string iterative_engine_v12 movetime_ms {}",
                ms
            ));
        }
        out.info_lines.push(format!(
