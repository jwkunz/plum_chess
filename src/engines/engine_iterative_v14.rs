//! Iterative-deepening material-search engine (V14).
//!
...
//!
//! V14 marker:
//! - Successor to V13 with tightened repetition-while-winning handling.
...
use crate::search::iterative_deepening_v14::{
...
impl Engine for IterativeEngine {
    ...
    fn choose_move(
        ...
        if root_legal.is_empty() {
            out.info_lines.push(
                "info string iterative_engine_v14 no legal root move in requested searchmoves"
                    .to_owned(),
            );
            return Ok(out);
        }
        ...
        if mate_mode.is_some() {
            if let Some(mate_one) = find_mate_in_one(game_state, &root_legal) {
                ...
                out.info_lines.push(
                    "info string iterative_engine_v14 mate_mode immediate_mate_selected".to_owned(),
                );
            }
        }
        ...
        if let Some(best) = chosen {
            ...
            if preferred != best {
                out.info_lines
                    .push("info string iterative_engine_v14 queen_promotion_preferred".to_owned());
            }
        ...
        out.info_lines.push(format!(
            "info string iterative_engine_v14 default_depth {}",
            self.default_depth
        ));
        out.info_lines.push(format!(
            "info string iterative_engine_v14 scorer {:?}",
            self.scorer_kind
        ));
        out.info_lines.push(format!(
            "info string iterative_engine_v14 used_depth {}",
            depth
        ));
        out.info_lines.push(format!(
            "info string iterative_engine_v14 control_mode {:?}",
            control_mode
        ));
        if let Some(mate) = mate_mode {
            out.info_lines.push(format!(
                "info string iterative_engine_v14 mate_mode plies_target {}",
                mate.saturating_mul(2).saturating_add(1)
            ));
        }
        if let Some(ms) = effective_params.movetime_ms {
            out.info_lines.push(format!(
                "info string iterative_engine_v14 movetime_ms {}",
                ms
            ));
        }
        out.info_lines.push(format!(
            "info string iterative_engine_v14 time_strategy {:?}",
            self.time_strategy
        ));
        out.info_lines.push(format!(
            "info string iterative_engine_v14 go_raw movetime={:?} wtime={:?} btime={:?} winc={:?} binc={:?}",
            params.movetime_ms, params.wtime_ms, params.btime_ms, params.winc_ms, params.binc_ms
        ));
        out.info_lines.push(format!(
            "info string iterative_engine_v14 go_modes nodes={:?} mate={:?} ponder={} infinite={}",
            params.nodes, params.mate, params.ponder, params.infinite
        ));
        if let Some(node_cap) = node_cap {
            out.info_lines.push(format!(
                "info string iterative_engine_v14 node_cap {}",
                node_cap
            ));
        }
        if params.ponder {
            out.info_lines.push(
                "info string iterative_engine_v14 note ponder mode parsed; search remains synchronous"
                    .to_owned(),
            );
        }
        if params.infinite {
            out.info_lines.push(
                "info string iterative_engine_v14 note infinite parsed; bounded iterative search is used in synchronous mode"
                    .to_owned(),
            );
        }
        out.info_lines.push(format!(
            "info string iterative_engine_v14 go_resolved movetime={:?}",
            effective_params.movetime_ms
        ));
        out.info_lines.push(format!(
            "info string iterative_engine_v14 movetime_source {}",
        ...
        {
            out.info_lines.push(
                "info string iterative_engine_v14 precedence movetime overrides nodes/mate"
                    .to_owned(),
            );
        } else if control_mode == GoControlMode::Nodes && params.mate.is_some() {
            out.info_lines.push(
                "info string iterative_engine_v14 precedence nodes overrides mate".to_owned(),
            );
        }
        out.info_lines.push(format!(
