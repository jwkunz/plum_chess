    ...
    if let Some(remaining) = remaining_opt {
        resolved.movetime_ms = Some(match strategy {
            ...
            TimeManagementStrategy::AdaptiveV13 => {
                adaptive_budget_ms(game_state, remaining, inc_opt)
            }
        });
    }
...
fn adaptive_budget_ms(game_state: &GameState, remaining_ms: u64, inc_ms: Option<u64>) -> u64 {
    let ply = u64::from(game_state.ply);
    let expected_moves_left = if ply < 20 {
        40
    } else if ply < 60 {
        28
    } else {
