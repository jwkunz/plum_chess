//! Iterative-deepening material-search engine (V5).
//!
...
//!
//! V5 marker:
//! - Successor to V4 with deeper pruning/stability improvements.
//! - Adds aspiration windows and null-move pruning in the search backend.
...
use crate::search::iterative_deepening_v5::{
...
impl Engine for IterativeEngine {
    ...
    fn choose_move(
        ...
        if let Some(best) = chosen {
            ...
            if preferred != best {
                out.info_lines
                    .push("info string iterative_engine_v5 queen_promotion_preferred".to_owned());
            }
        ...
        out.info_lines.push(format!(
            "info string iterative_engine_v5 default_depth {}",
            self.default_depth
        ));
        out.info_lines.push(format!(
            "info string iterative_engine_v5 scorer {:?}",
            self.scorer_kind
        ));
        out.info_lines.push(format!(
            "info string iterative_engine_v5 used_depth {}",
            depth
        ));
        if let Some(ms) = effective_params.movetime_ms {
            out.info_lines.push(format!(
                "info string iterative_engine_v5 movetime_ms {}",
                ms
            ));
        }
        out.info_lines.push(format!(
