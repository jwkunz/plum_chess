//! Iterative-deepening material-search engine (V8).
//!
...
//!
//! V8 marker:
//! - Successor to V7 with tactical-noise control in leaf search.
//! - Adds SEE-style tactical pruning/ordering in quiescence/captures.
...
use crate::search::iterative_deepening_v8::{
...
impl Engine for IterativeEngine {
    ...
    fn choose_move(
        ...
        if let Some(best) = chosen {
            ...
            if preferred != best {
                out.info_lines
                    .push("info string iterative_engine_v8 queen_promotion_preferred".to_owned());
            }
        ...
        out.info_lines.push(format!(
            "info string iterative_engine_v8 default_depth {}",
            self.default_depth
        ));
        out.info_lines.push(format!(
            "info string iterative_engine_v8 scorer {:?}",
            self.scorer_kind
        ));
        out.info_lines.push(format!(
            "info string iterative_engine_v8 used_depth {}",
            depth
        ));
        if let Some(ms) = params.movetime_ms {
            out.info_lines.push(format!(
                "info string iterative_engine_v8 movetime_ms {}",
                ms
            ));
        }
        out.info_lines.push(format!(
