//! Iterative-deepening material-search engine (V3).
//!
...
//!
//! V3 marker:
//! - Successor to V2 with endgame-focused search/eval heuristics.
//! - Uses tapered endgame scoring and repetition/check heuristics in search.
...
use crate::search::iterative_deepening_v3::{
...
impl Engine for IterativeEngine {
    ...
    fn choose_move(
        ...
        if let Some(best) = chosen {
            ...
            if preferred != best {
                out.info_lines
                    .push("info string iterative_engine_v2 queen_promotion_preferred".to_owned());
            }
        ...
        out.info_lines.push(format!(
            "info string iterative_engine_v3 default_depth {}",
            self.default_depth
        ));
        out.info_lines.push(format!(
            "info string iterative_engine_v3 scorer {:?}",
            self.scorer_kind
        ));
        out.info_lines.push(format!(
            "info string iterative_engine_v3 used_depth {}",
            depth
        ));
        if let Some(ms) = effective_params.movetime_ms {
            out.info_lines.push(format!(
                "info string iterative_engine_v3 movetime_ms {}",
                ms
            ));
        }
        out.info_lines.push(format!(
