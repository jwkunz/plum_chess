//! Iterative-deepening material-search engine (V4).
//!
...
//!
//! V4 marker:
//! - Successor to V3 with stronger search-ordering and pruning heuristics.
//! - Adds killer/history ordering and LMR in the search backend.
...
use crate::search::iterative_deepening_v4::{
...
impl Engine for IterativeEngine {
    ...
    fn choose_move(
        ...
        if let Some(best) = chosen {
            ...
            if preferred != best {
                out.info_lines
                    .push("info string iterative_engine_v4 queen_promotion_preferred".to_owned());
            }
        ...
        out.info_lines.push(format!(
            "info string iterative_engine_v4 default_depth {}",
            self.default_depth
        ));
        out.info_lines.push(format!(
            "info string iterative_engine_v4 scorer {:?}",
            self.scorer_kind
        ));
        out.info_lines.push(format!(
            "info string iterative_engine_v4 used_depth {}",
            depth
        ));
        if let Some(ms) = params.movetime_ms {
            out.info_lines.push(format!(
                "info string iterative_engine_v4 movetime_ms {}",
                ms
            ));
        }
        out.info_lines.push(format!(
