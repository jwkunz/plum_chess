//! Iterative-deepening material-search engine (V6).
//!
...
//!
//! V6 marker:
//! - Successor to V5 with stronger PV-aware search.
//! - Adds Principal Variation Search (PVS) in the search backend.
...
use crate::search::iterative_deepening_v6::{
...
impl Engine for IterativeEngine {
    ...
    fn choose_move(
        ...
        if let Some(best) = chosen {
            ...
            if preferred != best {
                out.info_lines
                    .push("info string iterative_engine_v6 queen_promotion_preferred".to_owned());
            }
        ...
        out.info_lines.push(format!(
            "info string iterative_engine_v6 default_depth {}",
            self.default_depth
        ));
        out.info_lines.push(format!(
            "info string iterative_engine_v6 scorer {:?}",
            self.scorer_kind
        ));
        out.info_lines.push(format!(
            "info string iterative_engine_v6 used_depth {}",
            depth
        ));
        if let Some(ms) = effective_params.movetime_ms {
            out.info_lines.push(format!(
                "info string iterative_engine_v6 movetime_ms {}",
                ms
            ));
        }
        out.info_lines.push(format!(
