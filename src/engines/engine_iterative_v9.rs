//! Iterative-deepening material-search engine (V9).
//!
...
//!
//! V9 marker:
//! - Successor to V8 with improved transposition-table retention quality.
//! - Uses TT generation aging and depth+age replacement in search backend.
...
use crate::search::iterative_deepening_v9::{
...
impl Engine for IterativeEngine {
    ...
    fn choose_move(
        ...
        if let Some(best) = chosen {
            ...
            if preferred != best {
                out.info_lines
                    .push("info string iterative_engine_v9 queen_promotion_preferred".to_owned());
            }
        ...
        out.info_lines.push(format!(
            "info string iterative_engine_v9 default_depth {}",
            self.default_depth
        ));
        out.info_lines.push(format!(
            "info string iterative_engine_v9 scorer {:?}",
            self.scorer_kind
        ));
        out.info_lines.push(format!(
            "info string iterative_engine_v9 used_depth {}",
            depth
        ));
        if let Some(ms) = params.movetime_ms {
            out.info_lines.push(format!(
                "info string iterative_engine_v9 movetime_ms {}",
                ms
            ));
        }
        out.info_lines.push(format!(
