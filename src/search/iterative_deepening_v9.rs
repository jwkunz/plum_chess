//! Iterative deepening search with negamax alpha-beta pruning (V9).
//!
...
fn negamax<S: BoardScorer>(
    ...
    *nodes += 1;
    ...
    if should_try_null_move(depth, in_check, beta, game_state) {
...
fn quiescence<S: BoardScorer>(
    ...
    *nodes += 1;
    ...
    moves.retain(|m| passes_quiescence_pruning(*m, stand_pat, alpha));
    order_moves_basic(&mut moves, None);
...
#[inline]
fn passes_quiescence_pruning(move_description: u64, stand_pat: i32, alpha: i32) -> bool {
    if move_promotion_piece_code(move_description) != NO_PIECE_CODE {
        return true;
    }

    let max_gain = capture_value(move_description) + promotion_gain(move_description);
    if stand_pat + max_gain + QUIESCENCE_DELTA_MARGIN < alpha {
        return false;
    }

    if (move_description & (FLAG_CAPTURE | FLAG_EN_PASSANT)) != 0
        && static_exchange_estimate(move_description) < SEE_BAD_CAPTURE_THRESHOLD
    {
        return false;
    }
