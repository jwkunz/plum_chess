//! Iterative deepening search with negamax alpha-beta pruning (V14).
//!
...
//!
//! V14 heuristics:
//! - Tightened repetition-while-winning draw penalty.
...
fn repetition_draw_score(static_eval_side_to_move: i32) -> i32 {
    // V14: strongly discourage repetition draws when winning. The previous
    // policy was symmetric and too draw-tolerant in clearly won positions.
    const WINNING_MARGIN: i32 = 40;
    const DRAW_PENALTY_BASE: i32 = 120;
    const DRAW_PENALTY_CAP: i32 = 1000;
    const DRAW_BONUS_BASE: i32 = 35;
    const DRAW_BONUS_CAP: i32 = 220;

    if static_eval_side_to_move > WINNING_MARGIN {
        let mut penalty = DRAW_PENALTY_BASE + (static_eval_side_to_move / 2).min(DRAW_PENALTY_CAP);
        if static_eval_side_to_move > 600 {
            penalty += 150;
        }
        -penalty
    } else if static_eval_side_to_move < -WINNING_MARGIN {
        let bonus = DRAW_BONUS_BASE + ((-static_eval_side_to_move) / 6).min(DRAW_BONUS_CAP);
        bonus
    } else {
        0
    }
