//! Iterative deepening search with negamax alpha-beta pruning (V5).
//!
...
fn negamax<S: BoardScorer>(
    ...
    *nodes += 1;
    ...
    for (move_index, mv) in moves.into_iter().enumerate() {
        ...
        let score_opt = if lmr_reduction > 0 {
...
fn order_moves(
    ...
    moves.sort_by_key(|m| -move_order_score(*m, tt_move, killers, history, side_to_move));
}
...
fn move_order_score(
    ...
    if is_quiet_move(move_description) {
        ...
        if let Some(piece) = piece_kind_from_code(move_moved_piece_code(move_description)) {
            score += history[side_to_move.index()][piece.index()][move_to_square(move_description)];
        }
    }
    score
}
