...
impl BoardLocation{
    pub fn from_long_algebraic(x : &str) -> Result<Self, ChessErrors>{
        ...
        Ok(BoardLocation::from_file_rank(file,rank))
    }
    pub fn from_file_rank(file : u8,rank : u8) -> Self{
        BoardLocation { binary_location : (1 as u64) << (8*file)+rank}
    ...
    pub fn generate_moved_location_checked(&self, d_file : i8, d_rank : i8) -> Result<BoardLocation,ChessErrors>{
        let (f,r) = self.get_file_rank();
        let f_next = f as i8 + d_file;
        if f_next < 0 || f_next > 7{
            return Err(ChessErrors::TriedToMoveOutOfBounds((self.clone(),d_file,d_rank)));
        }
        let r_next = r as i8 + d_rank;
        if r_next < 0 || r_next > 7{
            return Err(ChessErrors::TriedToMoveOutOfBounds((self.clone(),d_file,d_rank)));
        }        
        Ok(BoardLocation::from_file_rank(f as u8, r as u8))
    }    
    pub fn generate_moved_location_without_validation(&self, d_file : i8, d_rank : i8) -> BoardLocation{
        ...
        BoardLocation { binary_location:new_location }
    }              
}
