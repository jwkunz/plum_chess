...
## Current Status (v6.9.0)

- Steps `v6.0` through `v6.9` are implemented.
- Acceptance tooling is in place via `benches/v6_acceptance_criterion.rs`.
- Current measured results indicate opening/middlegame competitiveness, while
  endgame conversion/speed still leave room for future tuning.
...
10. `v6.9`: Add reproducible acceptance benchmarking (`v6_acceptance_criterion`) and from-state harness support to quantify v16 vs v17 strength/speed.
